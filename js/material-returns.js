...
async function downloadMaterialReturnPDF(){
  ...
  try{
    ...
    const drawHeader=()=>{
      ...
      doc.setFontSize(9.2);
      doc.text(number,W-margin-3.5,barY+5.7,{align:'right'});

      let infoY=barY+13;
      doc.setFont('helvetica','normal');
      doc.setFontSize(7.2);
      doc.setTextColor(...muted);
      doc.text(`PROJECT CODE  ${record.project_code||'–'}`,margin,infoY);

      const projectLines=doc.splitTextToSize(`PROJECT  ${record.project_name||'–'}`,104);
      doc.text(projectLines.slice(0,2),78,infoY);

      infoY+=8;
      doc.text(`DATE  ${record.return_date}`,margin,infoY);
      doc.text(`GENERATED BY  ${generatedBy}`,78,infoY);

      doc.setDrawColor(210,225,228);
      doc.setLineWidth(0.2);
      doc.line(margin,infoY+3,W-margin,infoY+3);
